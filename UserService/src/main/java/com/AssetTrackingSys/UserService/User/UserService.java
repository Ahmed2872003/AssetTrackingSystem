...
public class UserService {
    ...
    public List<User> getAllUsers(Long userId, User user){
        if(user.getRole() == "ASSET_MANAGER"){
            return userRepo.findAllWithAllowedRoles(userId, List.of(User.Roles.STAFF.getId()));
        }else{
            return userRepo.findAllExcludingRole(userId, User.Roles.ADMIN.getId());
        }
    }
